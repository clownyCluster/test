...
class OtpTextField extends StatefulWidget {
  ...
  OtpTextField({
    ...
    this.contentPadding,
        ...
        assert(styles.isEmpty || styles.length == numberOfFields);
...
class _OtpTextFieldState extends State<OtpTextField> {
  ...
  void initState() {
    ...
    _focusNodes = List<FocusNode?>.generate(widget.numberOfFields, (index) => FocusNode());
    _textControllers = List<TextEditingController?>.generate(widget.numberOfFields, (index) => TextEditingController());
  }
    ...
    return Container(
      ...
      child: TextFormField(
        ...
        onChanged: (String value) {
          ...
          changeFocusToNextNodeWhenValueIsEntered(value: value, indexOfTextField: index);
  ...
  void changeFocusToPreviousNodeWhenTapBackspace() async {
    ...
    try {
      ...
      if (index > 0 && (_textControllers[index]?.text.isEmpty ?? true)) {
  ...
  void onSubmit({required List<String?> verificationCode}) {
    if (verificationCode.every((String? code) => code != null && code!.isNotEmpty)) {
